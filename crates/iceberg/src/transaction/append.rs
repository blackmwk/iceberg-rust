...
pub struct FastAppendAction {
    ...
    // below are properties used to create a snapshot when committing
...
mod tests {
    ...
    async fn test_fast_append_preserves_delete_only_manifest() {
        ...
        let mut action_commit = Arc::new(action).commit(&mut state, &table).await.unwrap();
    ...
    async fn test_empty_data_append_action() {
        ...
        let mut state = SnapshotRetryState::default();
        assert!(Arc::new(action).commit(&mut state, &table).await.is_err());
    }
