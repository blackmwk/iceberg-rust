    ...
    fn clone(&self) -> Self {
        Self {
            ...
            actions: self
                .actions
                .iter()
                .map(TransactionActionEntry::clone_without_retry_state)
                .collect(),
        }
    }
}
...
impl Transaction {
    ...
    /// Create an action that atomically replaces data and delete files.
    ...
    async fn do_commit(&mut self, catalog: &dyn Catalog) -> Result<Table> {
        ...
        for entry in &mut self.actions {
            let action_commit = entry
                .action
                .commit(entry.retry_state.as_mut(), &current_table)
                .await?;
    ...
    async fn finish_actions(&mut self, cleanup_table: &Table, commit_error: Option<&Error>) {
        for entry in &mut self.actions {
            entry
                .action
                .finish_commit(entry.retry_state.as_mut(), cleanup_table, commit_error)
                .await;
        }
    }
}
